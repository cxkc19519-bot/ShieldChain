...
| IV-B.3b | User `U` | Provider | TLS handshake | Server-authenticated TLS using the Provider certificate/`PK_Prov` | User verifies the Provider certificate and public key; the paper does not authenticate the User by `Cert_U` or credentials during this handshake | Confidential, Provider-authenticated channel established; User account authentication remains pending until IV-B Step 5 | IV-B Step 3; Fig. 7 |
...
| IV-B.5 | External service `S` / Provider | Provider | Persistent identity/human-verification result associated with the Step 4 `<uid_U, passwd>` registration request | External identity mechanism; distinct from TLS channel authentication | Provider authenticates the registration identity through `S` and checks that the account does not exist; it does not rely on User-certificate mTLS | User registration accepted or rejected | IV-B Step 5; III-C; Fig. 7 |
...
| IV-C.4a | User `U` | Provider | TLS handshake | Server-authenticated TLS using the Provider identity established in IV-B Step 3 | User authenticates the Provider endpoint; the paper does not state User-certificate mTLS or Provider-side User account authentication in the handshake | Protected registration channel established; User account remains unauthenticated | IV-C Step 4; Fig. 8 |
| IV-C.4b | User `U` | Provider | `<uid_U, passwd>` | Account credentials sent inside the protected TLS channel | Provider verifies `<uid_U, passwd>` against `D_U`; this is account authentication, distinct from TLS channel authentication | User account authorized or rejected | IV-C Step 4; Fig. 8 |
...
| IV-E.8a | Agent `B` | Agent `A` | Subsequent request with attached `token` | TLS; `A` decrypts/validates Token under `SDHK` | Verify the Token was issued for B through its `PAC_B` binding, current time has not passed `T_expire`, and use has not reached `Q_max`. `T_issued` is carried as the issue timestamp, but the paper specifies no separate future-issued/not-before check | Valid request accepted and usage advances; invalid request rejected | IV-E Step 8; “Token reuse”; Fig. 9 |
