...
- **Consequence:** A failed delivery does not make the public OTK reissuable, and a consumed SOTK cannot generate a second ACT. Before the SOTK claim/delete linearization commit, the mapping remains available and `B` may retry with the same already allocated OTK. After that commit but before ACT persistence, the mapping is consumed and the OTK is lost; after ACT persistence but before `B` observes the response, `A` may retain an orphan ACT that `B` cannot use. Both post-claim cases, and every uncertain outcome at or after claim/delete, require a fresh OTK. The observable states are only available mapping, consumed mapping without stored ACT, or consumed mapping with stored/orphan ACT; no intention state is introduced. SOTK deletion and ACT creation are not one rollback-capable transaction, and distributed rollback/reconciliation is not invented as paper protocol.
