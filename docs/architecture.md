...
5. `A` atomically claims/deletes the SOTK mapping as its own irreversible linearization commit before creating any Token state. The claim/delete is never rolled back, even if later encryption, ACT persistence, or delivery fails.
6. Only after that commit, `A` creates exactly `<N, T_issued, T_expire, Q_max, PAC_B>`, encrypts it with fresh AEAD nonce/AAD, stores authoritative receiver state, and sends only the envelope/ciphertext to `B`.
...
- SOTK claim/delete as one irreversible linearization commit; subsequent receiver ACT-state creation is a separate commit and is not rolled back together with the SOTK;
...
Provider and receiving Agent transactions are separate trust/state domains. The architecture does not claim distributed atomic rollback between public OTK issuance and SOTK consumption. Once a public OTK is issued it is never returned to the Provider pool. At `A`, the three establishment crash points are explicit: (1) before the SOTK claim/delete commit, the mapping remains observably available and `B` may retry with that same already allocated OTK; (2) after claim/delete commits but before ACT persistence, the mapping is observably consumed and that OTK/SOTK pair is permanently lost, so retry requires a fresh OTK; (3) after ACT persistence but before the response is observed by `B`, the mapping is consumed and `A` may retain an orphan ACT that `B` cannot use because it never received the envelope, so retry also requires a fresh OTK. An uncertain outcome at or after the claim/delete linearization point is treated as consumed and requires a fresh OTK. There is no intermediate intention state, no path restores a committed deletion, and SOTK deletion plus ACT creation are never treated as one rollback-capable all-or-nothing transaction. Linearization tests cover both in-memory and SQLite adapters, including these three crash points, `q_max=1`, simultaneous last-budget use, and simultaneous last-OTK allocation. Crash tests do not claim atomicity between ACT-use-count commit and an external application side effect: without an outbox/idempotency extension, that boundary is at-most-one handler admission in a live process, not exactly-once effect delivery across crashes.
