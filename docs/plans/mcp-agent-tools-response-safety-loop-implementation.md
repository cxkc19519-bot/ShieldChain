...
| `response_planning/` | 严格候选 Schema、证据/目标重绑定、可信工具定义校验、服务端风险和审批计算、版本化计划四表 | 已接入运营响应规划；案件级计划动作可经操作员接受后映射到可信调用 |
...
为避免回滚静默删除新运行数据，只要 `operations_runs` 已存在记录，本迁移的 downgrade 就会明确失败。需要回滚时必须先备份并迁移这些运行和报告关联；不得通过关闭外键或直接删除表绕过该保护。
...
> 状态：已完成（2026-08-22）。现有 Broker 已具备 UTC/31 天窗口校验、稳定失败结果和单运行失败缓存；报告与前端不会把失败解释为空结果或无风险。
...
- 四类查询结果和证据限制不倒退；
- failed、empty、succeeded 明确区分；
- Provider 原始异常不进入 API、报告或前端；
- 同工具失败在单运行中只执行并缓存一次；
- 后端聚焦测试、完整前端测试、类型检查、Lint 和构建通过。
...
> 状态：已完成（2026-08-23）。新增通用父表与运营子表，历史调查运行以相同 ID 回填；阶段 4～6 的六个租户复合外键已重定向，旧调查仓储保持兼容并同步父状态。
...
- 新增 Alembic `20260823_01`，程序内真实执行 `20260729_01 → head → 20260729_01 → head` 通过；
- 回填测试覆盖旧 investigation 父行状态映射，并验证 case context、private context、handoff、execution、trusted tool 和 ReAct loop 六表在升降级前后各保持 1 行；
- 新运营报告在执行前创建 `agent_runs`/`operations_runs`，成功更新为 `completed`，受控异常更新为 `failed`；
- 报告 API 与前端公开 `run_id` 和 `run_status`，旧 JSON 报告明确显示 `legacy_without_run`；
- 迁移降级在存在运营运行时拒绝继续，避免静默删除新数据；
- 后端迁移、运营报告、旧调查仓储、Agent/Tool/ReAct 与 MCP 组合回归：`128 passed`；
- 前端全量：`27` 个测试文件、`96 passed`，TypeScript、ESLint 和生产构建通过；
- 排除可选 `torch` 测试的后端全量基线暴露出 1 个受本迁移影响的旧租户边界夹具，补入真实 `agent_runs` 父行后单测通过；其余失败仍来自已退役调查 API、旧健康检查契约、Windows 子进程路径、历史文档编码和容器合同，不在本 Task 中隐藏或顺带改写；
- 本 Task 不把旧调查专用仓储强行泛化为完整运营闭环；Agent Tool 审计、响应计划和 ReAct 编排仍由后续 Task 接线。

建议提交：`feat: introduce generic agent run persistence`

### Task 5：Agent Tool 调用审计

> 状态：已完成（2026-08-23）。内部运营工具和入站 MCP 调用使用同一持久化合同；`mcp_outbound` 方向已在仓储/Broker 合同中验证，但受控外部 MCP Client 尚未实现，不能表述为已有出站调用能力。
...
- 新增 Alembic `20260823_02` 和 `agent_tool_calls`，带租户复合外键、方向/状态/范围约束和两条查询索引；
- Broker 在 Provider 执行前独立提交 `running`，在成功、空结果、依赖失败或取消后提交明确终态；终态写入失败时不返回未经审计的可信结果；
- 应用启动把遗留 `running` 收敛为 `unknown/process_interrupted`，不猜测调用成功或失败；
- 内部运营报告记录真实 run、request ID 和首次调用角色；入站 MCP 记录服务器绑定的 tenant/principal，且不接受客户端覆盖身份；
- REST 只读投影为 `/api/v1/mcp/runs/{run_id}/calls`，按可信 tenant 过滤，不返回工具 items、原始载荷、Token、异常或堆栈；
- 三种 direction、依赖异常脱敏、恢复、公开投影、官方 MCP Client 和迁移升降升测试通过；
- 后端迁移、旧调查、Agent/可信工具/ReAct、运营报告、MCP 与 HTTP 安全组合回归：`158 passed`；
- downgrade 在已有审计行时拒绝删除表，回滚前必须先按留存要求备份并迁移审计记录；
- 当前 `references_json` 为空，因为四类内置工具尚未产生独立可信引用对象；不从文本 items 反推或伪造引用。
...
> 状态：已完成（2026-08-23）。本 Task 把案件级计划动作编译为可信调用并进入策略/审批等待边界；没有调度 Adapter，也没有宣称动作已执行或验证。
...
- 新增 `tools/plan_service.py` 和 `POST /api/v1/tools/plans/{plan_id}/decision`；tenant 与 actor 只取服务端身份，输入只允许 accepted/rejected 和受限原因文本；
- 接受仅适用于当前 `proposed`、有案件绑定且包含动作的 revision；拒绝不创建调用。重复接受/拒绝保持幂等，跨租户、终态计划、无动作建议和旧 revision 均拒绝；
- 每个动作重新读取同 run 已确认证据，复核 31 天时效、目标未变化、依赖只向前、工具/版本/风险/变更属性/验证器/参数仍与本地注册表一致；任一步失败整次事务回滚，计划保持 `proposed`；
- 可信请求保存真实 plan、plan revision 和 plan action ID；幂等键使用不可变 action ID 与工具版本，请求摘要继续使用现有规范化 `request_digest`，不额外计算文件或计划哈希；
- 接受后复用 `DeterministicToolPolicy` 在真实模式重新评估：只读动作可进入 `approved` 等待 Task 12 调度，高风险动作停在 `awaiting_approval`；计划接受本身不生成工具审批；
- 审批仍同时绑定 call ID、request digest、policy version 和有效期；参数变化产生不同摘要，旧审批不能授权新请求，已接受计划也不能被静默重新编译；
- 执行租约层新增计划依赖门禁：依赖调用必须为 `succeeded` 且存在 `verified` 回执；未验证依赖不能拿到租约。现有 CAS 和唯一活动租约约束继续保证同一调用最多一个执行租约；
- 新增 Alembic `20260823_06`：同 tenant 的每个 plan action 最多关联一个可信调用，计划操作员事件保存 actor subject；存在操作员审计事件时拒绝降级，避免静默删除主体信息；
- 可信工具轨迹增加可空 plan、plan revision 和 plan action 引用；历史调用保持关联为空；
- 计划接受/拒绝、跨租户、重复决策、策略/审批隔离、参数摘要、证据目标变化原子回滚、依赖验证、并发租约、迁移和 API 权威字段测试通过；Task 0～11 相关后端组合回归：`410 passed`；
- 当前尚无调度器把 `approved` 调用交给 Adapter，也没有把执行/验证回执送入 ReAct；这些属于 Task 12。
