...
ShieldChain 当前提供两类受控的实验室真实链路：宿主机 nftables 执行器负责 IP 查询、封禁和解封；端点链路负责查询 Wazuh 白名单 Agent，并在演示 Agent 自己的网络命名空间内执行隔离和恢复。模型只能从服务端候选中选择工具，目标仍由案件证据解析，不能自行填写 IP、Agent ID、命令或连接地址。
