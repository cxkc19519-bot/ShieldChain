...
4. 固定双语 RAG 评测测试；
5. PowerShell 脚本契约；
6. 阶段 3、阶段 4、阶段 5 与阶段 6 离线 smoke。
...
当前持久化只支持本机 SQLite，启动和 smoke 只面向单机开发；这不是并发生产数据库或部署方案。Docker 不是阶段 6 本地开发的前置条件。真实云/安全设备适配器、真实模型自主规划、授权环境验证和 Docker Compose 部署仍是后续工作。
