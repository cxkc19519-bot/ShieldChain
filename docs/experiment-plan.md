...
- **Warmup and samples:** three recorded warmup repetitions per scenario/point (excluded from primary summaries) use scheduling seeds `40998..41000`, then ten independent measured repetitions use scheduling seeds `41001..41010`. For both phases, repetition `r` within that phase visits points in the base-list order cyclically left-rotated by `(3*r) mod 7`. The warmup and measured arrays are disjoint and have lengths exactly three and ten. There is no adaptive stopping.
- **Aggregation unit:** for each repetition, total authorization-path duration (including every required reauthorization, Provider lookup, OTK, DH/HKDF, ACT creation, and validation) divided by the fixed denominator of 4,096 offered attempts. The point estimate is the median of the ten repetition means. The profile fixes `per_attempt_timeout_seconds=10` and `per_repetition_timeout_seconds=3600` (60 minutes).
...
- **Parameters:** lifetime in seconds `[1, 5, 30, 60, 300]`; a 600-second logical injected-clock window per repetition; mean request rate 10/s; `q_max=100_000` so quota never triggers; one initiating/receiving pair, one exact allow rule, sufficient OTK/budget, and a 1,024-byte payload in each fixed adapter/network scenario. For trace seed `s` and event index `i`, derive `u` from the first 53 bits of `SHA-256("SAGA-LIFETIME/v1" || uint64be(s) || uint64be(i))` mapped strictly into `(0,1)`, then use exponential inter-arrival `-ln(1-u)/10` seconds until the next event would exceed 600 seconds.
- **Warmup and samples:** the closed profile uses `workload_kind=fixed_duration`, `warmup_duration_seconds_per_repetition=600`, `measured_duration_seconds_per_repetition=600`, and null `warmup_offered_attempts_per_repetition`/`measured_offered_attempts_per_repetition`. Three recorded warmup traces use seeds `50998..51000`, then ten independent measured arrival traces use fixed seeds `51001..51010`; each seed's exact timestamp trace is saved and replayed for every lifetime. The fixed-seed trace generation determines the event count, which is recorded as a result and is never a stopping condition. For both phases, repetition `r` within that phase visits lifetimes in the base-list order cyclically left-rotated by `(2*r) mod 5`; `seed_scope=trace_generation` and `failure_rule=fail_profile_no_replacement`. The profile fixes `per_attempt_timeout_seconds=10` and `per_repetition_timeout_seconds=3600`. The 600-second values are logical/injected-clock workload windows, whereas 3,600 seconds is only the wall-clock execution ceiling for a repetition; neither may be substituted for the other or adjusted after observing results. No wall-clock sleeps or adaptive stopping are allowed.
...
- **Pass rule:** all paired adjacent comparisons are non-increasing, `ratio <= 0.20`, and the upper CI bound for `ratio` is `< 0.50`. A quota-driven renewal, primitive/protocol failure, attempt/repetition timeout, missing or extra trace event, selector mismatch, or replacement attempt fails the repetition and profile; no replacement repetition or post-observation extension is allowed.
...
- **Warmup and samples:** two recorded 15-second warmup intervals per cell use target-order seeds `60999..61000`, then eight independent 30-second measured repetitions use target-order seeds `61001..61008`. Number the nine `(agent_count, concurrency)` cells in the written Cartesian-product order; for both phases, repetition `r` within that phase visits cells in the base order cyclically left-rotated by `(4*r) mod 9`. The profile fixes `per_attempt_timeout_seconds=10` and `per_repetition_timeout_seconds=120`; the repetition timeout is strictly greater than both the 15-second warmup and 30-second measured windows and may not be changed from observed results. No adaptive extension is allowed.
...
- **Repeatability pass rule:** every cell has throughput coefficient of variation `<= 0.15`; its bootstrap 95% CI half-width divided by median throughput is `<= 0.15`; and every concurrency receives a non-inconclusive direction classification. A primitive/protocol failure, attempt/repetition timeout, missing/extra interval, selector mismatch, replacement attempt, metric gap, or telemetry gap fails the repetition and profile; no replacement repetition or post-observation extension is allowed. The report must explain the classified direction using the measured CPU, lock, TLS, latency, and failure telemetry without changing the classification.
...
Sweep closed-loop client concurrency `[1, 2, 4, 8, 16, 32, 64]` for two fixed 1,024-byte mixes: authorization-heavy (`q_max=1`) and ACT-reuse-heavy (`q_max=64`), both with 10-minute lifetime and sufficient OTK/pair budget. Use two recorded 15-second warmups with scheduling seeds `70999..71000` and eight fixed 30-second measured repetitions per point with scheduling seeds `71001..71008`; for both phases, repetition `r` within that phase visits the base concurrency list cyclically left-rotated by `(3*r) mod 7`. The profile fixes `per_attempt_timeout_seconds=10` and `per_repetition_timeout_seconds=120`; the repetition timeout is strictly greater than both duration windows and may not be changed from observed results. There is no adaptive stopping. The aggregation unit is one repetition/point. Measure successful requests/second, offered attempts, rejection/error class, median/P95/P99 latency, CPU/memory, SQLite lock retries, and connection/TLS overhead. Open-loop workloads, if added, are separate scenarios and cannot replace this gate. Saturation and failures remain recorded samples. A primitive/protocol failure, attempt/repetition timeout, missing/extra interval, selector mismatch, or replacement attempt fails the repetition and profile; no replacement repetition or post-observation extension is allowed.
...
| `execution_profile_id` | string | required, non-null | Resolves exactly one `configuration.execution_profiles[].profile_id` whose family and selector match this sample |
...
`benchmark-summary.csv` contains exactly one row per `(run_id, scenario, execution_profile_id, operation, canonical parameters, environment_fingerprint, adapter, concurrency)` measured group. Duration statistics use successful measured attempts only; failed-attempt durations remain in `benchmark-samples.csv`, while counts preserve the full offered workload:

| Field | Type/unit | Required/nullable | Rule |
|---|---|---|---|
| `schema_version`, `run_id`, `scenario`, `execution_profile_id`, `operation`, `parameters`, `environment_fingerprint`, `adapter`, `concurrency` | as above | required, non-null | Exact grouping key; profile must resolve and select this operation/parameter point |
...
`configuration` has exactly these required, non-null properties. Execution controls are intentionally profile-local: there is no global `warmup`, `measured`, `scheduling`, or `timeouts` object whose values could be incorrectly applied to heterogeneous operations.
...
| `parameters` | object | Canonical typed family parameters with unit suffixes where dimensional; family validator rejects an incomplete/extra parameter point |
| `adapters` | unique array of string enum | Each value `memory`, `sqlite`, or `network`; written execution order |
| `execution_profiles` | non-empty array of closed execution-profile objects | Unique `profile_id`; profiles collectively select every configured operation/parameter point exactly once and no unconfigured point |
...
| `pass_rules` | non-empty array of non-empty strings | Exact preregistered Boolean gate rules in addition to the per-profile failure rule |

Each `configuration.execution_profiles[]` object is closed and has exactly these required properties; nullable properties must be present with `null`:

| Property | Type/unit | Nullable | Exact rule |
|---|---|---|---|
| `profile_id` | string matching `^[a-z][a-z0-9_.-]{2,63}$` | no | Stable and unique within the configuration; referenced by every matching sample and summary |
| `benchmark_family` | same family enum as configuration | no | Must equal `configuration.benchmark_family` |
| `selectors` | non-empty array of closed selector objects | no | Every selector has exactly `operation` (the sample-operation enum) and `parameter_point` (closed canonical JSON object); selector pairs are unique across all profiles |
| `warmup_seeds` | array of non-negative integers | no | Length exactly `warmup_repetitions`; unique within this array |
| `measured_seeds` | array of non-negative integers | no | Length exactly `measured_repetitions`; unique within this array and disjoint from `warmup_seeds` |
| `warmup_repetitions` | integer >= 0 | no | Fixed before execution |
| `measured_repetitions` | integer >= 1 | no | Fixed before execution |
| `workload_kind` | string enum | no | `fixed_attempts` or `fixed_duration` |
| `warmup_offered_attempts_per_repetition`, `measured_offered_attempts_per_repetition` | integer >= 1 | yes | Both non-null exactly for `fixed_attempts`; fixed phase-specific denominator for every selector/point in the profile |
| `warmup_duration_seconds_per_repetition`, `measured_duration_seconds_per_repetition` | finite number > 0, seconds | yes | Both non-null exactly for `fixed_duration`; offered attempts are observed rather than a stopping target |
| `per_attempt_timeout_seconds` | finite number > 0, seconds | no | Fixed timeout for every offered attempt |
| `per_repetition_timeout_seconds` | finite number > 0, seconds | no | Hard timeout greater than the fixed-duration window when applicable |
| `scheduling_order_formula` | non-empty string | no | Exact deterministic selector/point order formula, including repetition-index convention |
| `seed_scope` | string enum | no | `schedule_only`, `schedule_and_fixture`, or `trace_generation` |
| `failure_rule` | string enum | no | Exactly `fail_profile_no_replacement`: any primitive/protocol failure, timeout, missing/extra attempt, selector mismatch, or replacement attempt fails the repetition/profile; no adaptive stopping or extra repetition |

For `fixed_attempts`, completed-run validation requires exactly the phase-specific offered-attempt count for each selected operation/parameter point, phase, and corresponding seed/repetition. For `fixed_duration`, it requires the exact phase-specific duration/seed schedule and preserves every offered attempt; the offered count is an outcome and cannot be used for data-dependent stopping. Every `samples[].execution_profile_id` and `summaries[].execution_profile_id` must resolve exactly one profile, and its `operation` plus canonical `parameters` must match exactly one selector in that profile. The validator rejects unreferenced profiles, samples/summaries without profiles, cross-profile selector overlap, repetition/seed/count mismatches, and summaries that combine profile IDs.

The fixed benchmark tables above map to profiles as follows. Ordinary crypto operations use `3 × 1,024`, `10 × 4,096`, seeds `10998..11000`/`11001..11010`, and `2 s / 60 min`; scrypt alone uses `2 × 8`, `8 × 32`, seeds `11999..12000`/`12001..12008`, and `30 s / 20 min`. Registration/authorization uses three profiles corresponding exactly to its three table rows, preserving their distinct attempts, repetition counts, seeds, and timeouts. Each adapter/scenario has one closed profile per trend family because all points within that family share controls:

- q_max is `fixed_attempts`, `3 × 4,096`/`10 × 4,096`, seeds `40998..41000`/`41001..41010`, and `10 s / 3,600 s`.
- Token lifetime is `fixed_duration`, with `3 × 600 s` logical warmup windows and `10 × 600 s` logical measured windows, null offered-attempt targets, seeds `50998..51000`/`51001..51010`, and `10 s / 3,600 s` attempt/wall-clock timeouts. Each 600-second value is an injected-clock duration and 3,600 seconds is a distinct wall-clock upper bound; the seed-generated and recorded event count is an outcome, not a fixed denominator or stopping rule.
- Agent scale is `fixed_duration`, `2 × 15 s`/`8 × 30 s`, seeds `60999..61000`/`61001..61008`, and `10 s / 120 s`.
- Concurrent throughput is `fixed_duration`, `2 × 15 s`/`8 × 30 s`, seeds `70999..71000`/`71001..71008`, and `10 s / 120 s`.

In every case the first timeout is per attempt, the second per repetition, selectors cover each configured point exactly once, the written phase-local rotation formula fixes order, `seed_scope` is `trace_generation` for token lifetime and `schedule_only` for the other three families, and `failure_rule` is `fail_profile_no_replacement`.
...
Selector `parameter_point` objects are closed and exact: crypto key-generation operations use `{}`; Ed25519 sign/verify use exactly `message_bytes`; X25519 exchange uses `{}`; HKDF uses exactly `input_bytes,output_bytes`; ACT encrypt/decrypt use exactly `act_plaintext_fields`; certificate parse/verify use exactly `certificate_path_length`; and scrypt uses exactly `scrypt_n,scrypt_r,scrypt_p,scrypt_dklen_bytes`, with values/types constrained by the family parameters above. Registration/authorization uses `{}` for User registration, exactly `otk_batch_size` for Agent registration and signed-OTK append, and exactly `payload_bytes,lifetime_ms,q_max` for contact resolution/ACT establishment/ACT validation-use. Trend-family selectors use `application_authorization` and exactly the varied point keys: `q_max`, `lifetime_seconds`, `receiving_agent_count,concurrency`, or `q_max,concurrency`, respectively; values must belong to their family arrays. No selector may carry adapter, seed, timeout, or repetition controls.

Each JSON `samples[]` object contains exactly the 18 columns of `benchmark-samples.csv` with the same required/nullability rules; `parameters` is an object rather than a CSV string. `operation` is enum `ed25519_keygen`, `ed25519_sign`, `ed25519_verify`, `x25519_keygen`, `x25519_exchange`, `hkdf_sha256`, `act_encrypt`, `act_decrypt`, `certificate_parse`, `certificate_verify`, `scrypt`, `user_registration`, `agent_registration`, `signed_otk_append`, `contact_resolution`, `act_establishment`, `act_validation_use`, or `application_authorization`. Nullable `error_class` is null iff success, otherwise enum `InvalidInput`, `AuthenticationFailed`, `InvalidCertificate`, `InvalidSignature`, `DuplicateRegistration`, `PolicyDenied`, `PolicyNoMatch`, `PairBudgetExhausted`, `OtkPoolExhausted`, `OtkConsumed`, `AgentInactive`, `TokenIdentityMismatch`, `TokenNotYetValid`, `TokenExpired`, `TokenExhausted`, `InvalidTokenCiphertext`, `ConcurrentConflict`, `Timeout`, or `InternalError`. `scenario` is a non-empty registered name; all other sample types/units are exactly the sample CSV table above.

Each JSON `summaries[]` object contains exactly the 18 expanded columns of `benchmark-summary.csv`: the nine grouping keys, five nullable nanosecond statistics, three non-null counts, and nullable `throughput_per_second`; `parameters` is an object. Nullability and equations are exactly those in the CSV table, and no trend/gate property is permitted.
...
```json
{
  ...
  "configuration": {
    "benchmark_family":"crypto_microbenchmark",
    "parameters":{"operations":["ed25519_keygen","scrypt"],"message_bytes":1024,"hkdf_input_bytes":32,"hkdf_output_bytes":32,"act_plaintext_fields":["nonce","issued_at","expires_at","q_max","initiating_agent_access_control_public_key"],"certificate_path_length":2,"scrypt_n":32768,"scrypt_r":8,"scrypt_p":1,"scrypt_dklen_bytes":32,"setup_included":false},
    "adapters":["memory"],
    "execution_profiles":[
      {"profile_id":"crypto.ordinary","benchmark_family":"crypto_microbenchmark","selectors":[{"operation":"ed25519_keygen","parameter_point":{}}],"warmup_seeds":[10998,10999,11000],"measured_seeds":[11001,11002,11003,11004,11005,11006,11007,11008,11009,11010],"warmup_repetitions":3,"measured_repetitions":10,"workload_kind":"fixed_attempts","warmup_offered_attempts_per_repetition":1024,"measured_offered_attempts_per_repetition":4096,"warmup_duration_seconds_per_repetition":null,"measured_duration_seconds_per_repetition":null,"per_attempt_timeout_seconds":2,"per_repetition_timeout_seconds":3600,"scheduling_order_formula":"left_rotate(repetition_index mod operation_count)","seed_scope":"schedule_only","failure_rule":"fail_profile_no_replacement"},
      {"profile_id":"crypto.scrypt","benchmark_family":"crypto_microbenchmark","selectors":[{"operation":"scrypt","parameter_point":{"scrypt_n":32768,"scrypt_r":8,"scrypt_p":1,"scrypt_dklen_bytes":32}}],"warmup_seeds":[11999,12000],"measured_seeds":[12001,12002,12003,12004,12005,12006,12007,12008],"warmup_repetitions":2,"measured_repetitions":8,"workload_kind":"fixed_attempts","warmup_offered_attempts_per_repetition":8,"measured_offered_attempts_per_repetition":32,"warmup_duration_seconds_per_repetition":null,"measured_duration_seconds_per_repetition":null,"per_attempt_timeout_seconds":30,"per_repetition_timeout_seconds":1200,"scheduling_order_formula":"single_selector","seed_scope":"schedule_only","failure_rule":"fail_profile_no_replacement"}
    ],
    "statistics":{"aggregation_unit":"operation_repetition","quantile_method":"hyndman_fan_type_7","standard_deviation":"sample_n_minus_1","bootstrap_resamples":0,"bootstrap_seed":null},"tolerances":{},"pass_rules":["all execution profiles pass fail_profile_no_replacement"]
  },
  "samples":[],"summaries":[],"trend_gates":[],"comparison":[]
}
```

This example must fail schema validation: the duplicate `profile_id` is not stable-unique, the first profile omits required `warmup_seeds`, the second profile declares two measured repetitions but supplies one measured seed, and its selector repeats the same operation/parameter point across profiles. It remains syntactically valid JSON so parsing success must not be reported as schema-validation success.

```json
{"schema_version":"saga-benchmark/v1","run":{"run_id":"00000000-0000-4000-8000-000000000002","timestamp_utc":"2026-07-15T00:00:00Z","git_commit":"0000000000000000000000000000000000000000","dirty":false},"environments":{"41267dcc00ab22f82bcfc8e3bc423069d56b5237b522fbea3616828f89845d1b":{"cpu_model":"Example CPU","cpu_logical_count":8,"cpu_physical_count":4,"memory_bytes":17179869184,"os_name":"ExampleOS","os_version":"1","kernel_version":"6.8.0","architecture":"x86_64","python_implementation":"CPython","python_version":"3.13.5","dependency_lock_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","dependency_versions":{},"tls_backend":"OpenSSL","tls_version":"3.5.0","sqlite_version":"3.50.1","sqlite_journal_mode":null,"network_topology":"loopback_single_host","storage_type":"memory","storage_location_class":"ram","power_mode":"performance","proverif_version":null,"canonical_fingerprint_input":"{\"architecture\":\"x86_64\",\"cpu_logical_count\":8,\"cpu_model\":\"Example CPU\",\"cpu_physical_count\":4,\"dependency_lock_sha256\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",\"dependency_versions\":{},\"kernel_version\":\"6.8.0\",\"memory_bytes\":17179869184,\"network_topology\":\"loopback_single_host\",\"os_name\":\"ExampleOS\",\"os_version\":\"1\",\"power_mode\":\"performance\",\"proverif_version\":null,\"python_implementation\":\"CPython\",\"python_version\":\"3.13.5\",\"sqlite_journal_mode\":null,\"sqlite_version\":\"3.50.1\",\"storage_location_class\":\"ram\",\"storage_type\":\"memory\",\"tls_backend\":\"OpenSSL\",\"tls_version\":\"3.5.0\"}"}},"configuration":{"benchmark_family":"crypto_microbenchmark","parameters":{"operations":["ed25519_keygen"],"message_bytes":1024,"hkdf_input_bytes":32,"hkdf_output_bytes":32,"act_plaintext_fields":["nonce","issued_at","expires_at","q_max","initiating_agent_access_control_public_key"],"certificate_path_length":2,"scrypt_n":32768,"scrypt_r":8,"scrypt_p":1,"scrypt_dklen_bytes":32,"setup_included":false},"adapters":["memory"],"execution_profiles":[{"profile_id":"crypto.same","benchmark_family":"crypto_microbenchmark","selectors":[{"operation":"ed25519_keygen","parameter_point":{}}],"measured_seeds":[1],"warmup_repetitions":1,"measured_repetitions":1,"workload_kind":"fixed_attempts","warmup_offered_attempts_per_repetition":1,"measured_offered_attempts_per_repetition":1,"warmup_duration_seconds_per_repetition":null,"measured_duration_seconds_per_repetition":null,"per_attempt_timeout_seconds":2,"per_repetition_timeout_seconds":60,"scheduling_order_formula":"single_selector","seed_scope":"schedule_only","failure_rule":"fail_profile_no_replacement"},{"profile_id":"crypto.same","benchmark_family":"crypto_microbenchmark","selectors":[{"operation":"ed25519_keygen","parameter_point":{}}],"warmup_seeds":[2],"measured_seeds":[3],"warmup_repetitions":1,"measured_repetitions":2,"workload_kind":"fixed_attempts","warmup_offered_attempts_per_repetition":1,"measured_offered_attempts_per_repetition":1,"warmup_duration_seconds_per_repetition":null,"measured_duration_seconds_per_repetition":null,"per_attempt_timeout_seconds":2,"per_repetition_timeout_seconds":60,"scheduling_order_formula":"single_selector","seed_scope":"schedule_only","failure_rule":"fail_profile_no_replacement"}],"statistics":{"aggregation_unit":"operation_repetition","quantile_method":"hyndman_fan_type_7","standard_deviation":"sample_n_minus_1","bootstrap_resamples":0,"bootstrap_seed":null},"tolerances":{},"pass_rules":["all profiles pass"]},"samples":[],"summaries":[],"trend_gates":[],"comparison":[]}
```
