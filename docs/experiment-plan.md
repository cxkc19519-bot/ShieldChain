...
- **Warmup and samples:** three recorded warmup traces with seeds `50998..51000`, then ten independent measured arrival traces generated from fixed seeds `51001..51010`; each seed's exact timestamp trace is saved and replayed for every lifetime. For both phases, repetition `r` within that phase visits lifetimes in the base-list order cyclically left-rotated by `(2*r) mod 5`. The profile fixes `per_attempt_timeout_seconds=10` and `per_repetition_timeout_seconds=3600`; these bounds apply to the fixed arrival-trace workload and may not be adjusted after observing results. No wall-clock sleeps or adaptive stopping are allowed.
