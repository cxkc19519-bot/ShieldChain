...
|---|---|---|
| **C1 - registered adversarial Agent** | An adversary can create and register an Agent under its own identity, deviate from the protocol, and use social engineering to persuade a benign User to allow it. | Accepted in III-D. IV-F limits subsequent access through authenticated registration, the receiving Agent's Contact Policy, a bounded ACT lifetime/quota, and later policy blocking. Appendix E exercises C1 in A1-A6 and A8. The social-engineering success itself is not prevented. |
| **C2 - compromised legitimate Agent** | A registered legitimate Agent can be compromised through websites, tools, or other external resources and can then use the Agent's identity and existing credentials/Tokens. | Accepted in III-D and explicitly not claimed preventable in IV-F. IV-D.2 limits each contact-resolution response to one OTK; IV-E Steps 7-8 and Token reuse bound subsequent use by expiration and `Q_max`. Appendix E exercises C2 in A1-A6 and A8. |
| **C3 - unregistered self-replicated child** | A parent Agent can create an unregistered child and share TLS keys, access-control keys, and existing ACTs with it. | Accepted in III-D. IV-B Step 5 prevents the child from independently registering without human verification; IV-F states the child remains within the parent's identity and capabilities, not that SAGA detects or removes the child. Appendix E maps C3 to A5-A8. |
| **C4 - credential or Token sharing** | An adversarial Agent can share its TLS credentials, access-control keys, and ACTs with another adversary-controlled Agent. | Accepted in III-D. IV-F accepts that the recipient gains the donor Agent's capabilities but limits use by policy, expiration, quota, and deactivation/blocking for future contact. IV-E Step 8 rejects a Token whose `PAC_B` binding does not match the authenticated initiator. Appendix E maps C4 to A5, A6, and A8. |
...
The paper reports ProVerif results for only the following scope (IV-F; Appendix D):
...
4. **Reachability:** event queries establish that the modeled protocol roles can reach the relevant authentication/communication events.
