...
| **C5 - Sybil identities** | An adversary may try to create Agents under multiple identities. | Defined in III-D. III-C's Secure User Authentication and Human Verification and Agent Identity Control assumptions, together with IV-B Step 5, require authenticated human registration and prevent registration under a benign User's identity; Appendix E A7 evaluates rejection of autonomous copy registration. IV-C Step 6 separately checks each submitted Agent ID for uniqueness, but unique Agent IDs do not prevent one adversary from obtaining multiple identities and are not a Sybil defense. Table IV does not map an A1-A8 case directly to C5. |
| **C6 - network-level Dolev-Yao attacker** | The attacker can overhear, intercept, modify, replay, reorder, and synthesize arbitrary network messages, subject to the symbolic hardness of modeled cryptography. | Defined in III-D. Appendix D applies this attacker to symbolic Token-secrecy and authentication queries. At runtime, III-C separately assumes TLS defeats network tampering, eavesdropping, and record replay. Appendix E describes message-level manipulation in its evaluation scope but Table IV's A1-A8 capability column does not assign a row to C6. |
