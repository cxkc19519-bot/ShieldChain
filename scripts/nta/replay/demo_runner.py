...
class DemoReplayRunner:
    def __init__(self, *, manifest: Path, pcap_root: Path, runtime_root: Path) -> None:
        ...
        self._samples_cache: tuple[DemoSample, ...] | None = None
    ...
    def _samples(self) -> tuple[DemoSample, ...]:
        ...
        for row in rows:
            ...
            if not all(
                ...
                for item in (sample_id, relative, title, digest)
            ):
                raise ValueError(
                    "demo replay sample requires id, path, title and sha256"
                )
            ...
            if (
                not path.is_relative_to(self._pcap_root)
                or path.suffix.casefold() != ".pcap"
            ):
                raise ValueError("demo replay sample is outside the allowed PCAP root")
            if sample_id in sample_ids or path in sample_paths:
                raise ValueError("demo replay manifest contains a duplicate sample")
            ...
            samples.append(
                DemoSample(sample_id, path, title, digest.casefold(), tuple(rule_ids))
            )
            sample_ids.add(sample_id)
            sample_paths.add(path)
        self._samples_cache = tuple(samples)
        return self._samples_cache

    def status(self) -> dict[str, Any]:
        with self._lock:
            if self._status["state"] == "idle" and self._status["sample_count"] is None:
                try:
                    self._status["sample_count"] = len(self._samples())
    ...
    def _run(self, sample: DemoSample, run_id: str) -> None:
        ...
        try:
            ...
            if not rule_ids.intersection(sample.expected_rule_ids):
                raise RuntimeError(
                    "replay completed but expected detection was not observed"
                )
    ...
    class Handler(BaseHTTPRequestHandler):
        def _authorized(self) -> bool:
            return hmac.compare_digest(
                self.headers.get("X-ShieldChain-Nta-Replay-Token", ""), token
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="ShieldChain allowlisted demo replay runner"
    )
    ...
    if len(token) < 24:
        raise ValueError(
            "SHIELDCHAIN_NTA_REPLAY_RUNNER_TOKEN must contain at least 24 characters"
        )
    serve(
        DemoReplayRunner(
