...
async function start() {
  fireEvent.click(screen.getByRole('button', { name: '启动调查' }))
...
describe('InvestigationPage', () => {
  it('runs the default simulation to a verified closed result at 500 ms intervals', async () => {
    api.getInvestigation
      ...
      .mockResolvedValueOnce(run('closed', {
        steps: [{ step_key: 'collect_evidence', status: 'completed', detail: {}, error_code: null, started_at: NOW, completed_at: NOW }],
        ...
        assessment: { conclusion: '恶意外连已确认', risk_level: 'high', rule_ids: ['rule-1'], evidence_ids: [ID], recommended_action: 'block', explanation: '证据链支持阻断' },
    ...
    await start()
    expect(screen.getByRole('button', { name: '启动调查' })).toBeDisabled()
    expect(screen.getByRole('button', { name: '重置场景' })).toBeDisabled()
  ...
  it.each([
    null,
    { blocked: false, connection_stopped: true, observed_at: NOW, evidence_ids: [] },
    { blocked: true, connection_stopped: false, observed_at: NOW, evidence_ids: [] },
  ])('does not claim closure when closed has incomplete verification: %o', async (verification) => {
    api.startInvestigation.mockResolvedValue(run('closed', { verification }))
    render(<InvestigationPage />)
    await start()

    expect(screen.queryByText('已闭环')).not.toBeInTheDocument()
    expect(screen.queryByText('closed')).not.toBeInTheDocument()
    expect(screen.getByText('闭环未验证')).toBeVisible()
  })
  ...
  it('reset aborts page-owned polling and loads a fresh scenario', async () => {
    ...
    expect(screen.queryByText('处置失败')).not.toBeInTheDocument()
    expect(screen.getByText('模拟环境')).toBeVisible()
  })
  ...
  it('requires a fresh reset after an independent reset fails', async () => {
    const freshScenario = {
    ...
    fireEvent.click(screen.getByRole('button', { name: '重置场景' }))
    await act(async () => undefined)
    api.resetPhishingScenario.mockClear()
    api.resetPhishingScenario
      .mockRejectedValueOnce(new Error('reset unavailable'))
      .mockResolvedValueOnce(freshScenario)
    ...
    expect(api.resetPhishingScenario).toHaveBeenCalledTimes(2)
    expect(api.startInvestigation).toHaveBeenCalledWith(FRESH_ID, 'normal', expect.any(AbortSignal))
    expect(api.startInvestigation).not.toHaveBeenCalledWith(ID, expect.anything(), expect.anything())
  })
  ...
  it('shows tool failure without a false success state', async () => {
    ...
    fireEvent.change(screen.getByLabelText('调查模式'), { target: { value: 'fail_block_once' } })
    await start()
