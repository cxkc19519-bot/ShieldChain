...
type ModelId = 'qwen3' | 'qwen38'
type Catalog = { ready: boolean; active_model: ModelId | null; target_model: ModelId | null; phase: string; message: string; can_switch: boolean }
const modelNames: Record<ModelId, string> = { qwen3: 'Qwen3-30B-A3B-Instruct-2507-FP8', qwen38: 'Qwen3.8-27B-FP8' }
...
function restoreMessages(model: ModelId = 'qwen3'): Message[] {
  try {
    const raw: unknown = JSON.parse(localStorage.getItem(`${STORAGE_KEY}-${model}`) || (model === 'qwen3' ? localStorage.getItem(STORAGE_KEY) : null) || '[]')
...
export function QwenChatPage() {
  const [selected, setSelected] = useState<ModelId>('qwen3')
  const [histories, setHistories] = useState<Record<ModelId, Message[]>>(() => ({ qwen3: restoreMessages('qwen3'), qwen38: restoreMessages('qwen38') }))
  const messages = histories[selected]
  const setMessages = (value: Message[] | ((current: Message[]) => Message[])) => setHistories((current) => ({ ...current, [selected]: typeof value === 'function' ? value(current[selected]) : value }))
  ...
  const ready = catalog?.ready === true && catalog.active_model === selected && !switchPending
  const switching = switchPending || ['queued', 'switching', 'loading', 'rollback'].includes(catalog?.phase || '')
  ...
  useEffect(() => {
    try {
      for (const model of ['qwen3', 'qwen38'] as const) localStorage.setItem(`${STORAGE_KEY}-${model}`, JSON.stringify(histories[model]))
    } catch { /* Keep the current conversation usable if storage is full. */ }
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        const result = await api<Catalog>('/models', { signal: controller.signal })
        if (!cancelled) {
          setCatalog(result)
          setStatus({ ready: result.ready, model: result.active_model ? modelNames[result.active_model] : '', provider: 'local-qwen' })
          if (!initialized.current) {
            if (result.active_model) setSelected(result.active_model)
  ...
  async function activate() {
    if (pending || switching || !catalog?.can_switch) return
    if (!window.confirm('切换会暂停共享模型服务，加载可能需要数分钟。两个模型的对话记录会分别保留。继续？')) return
    setSwitchPending(true)
    setError(null)
    try {
      setCatalog(await api<Catalog>('/select', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: selected }) }))
    } catch (reason) {
    ...
    <div className="qwen-lab">
      ...
      <main className="qwen-stage">
        <header className="qwen-header">
          <div><span>本地大模型体验</span><h1>模型对比测试</h1></div>
          <div className="qwen-direct">{ready ? '模型已就绪' : '所选模型未就绪'}</div>
        </header>

        <section className="qwen-model-picker" aria-label="模型选择">
          <label htmlFor="qwen-model-select">测试模型</label>
          <select id="qwen-model-select" value={selected} disabled={pending || switching}
            onChange={(event) => { setSelected(event.target.value as ModelId); setError(null) }}>
            {Object.entries(modelNames).map(([id, name]) => <option key={id} value={id}>{name}{catalog?.active_model === id && catalog.ready ? ' · 运行中' : ' · 已下载'}</option>)}
          </select>
          <button type="button" onClick={() => void activate()} disabled={ready || pending || switching || !catalog?.can_switch}>
            {switching ? '正在切换…' : ready ? '运行中' : '启动所选模型'}
          </button>
          <p role="status">{catalog ? catalog.message : '模型状态暂不可用，正在重试…'}{catalog?.can_switch && !ready && !switching ? '。选择后点击“启动所选模型”。' : ''}</p>
          <small>双卡共享，模型轮流运行；切换会暂停其他模型功能。各模型独立保存对话，相同问题可重复测试。当前为文本、非思考模式。</small>
        </section>

        {messages.length === 0 ? (
          <section className="qwen-welcome">
            <div className="qwen-orb"><Bot size={34} /></div>
            <p><Sparkles size={16} />{modelNames[selected]}</p>
            <h2>想测试什么能力？</h2>
            ...
            <div className="qwen-suggestions">
              {suggestions.map((suggestion) => (
                <button type="button" key={suggestion} onClick={() => void send(undefined, suggestion)}
                  disabled={pending || !ready}>{suggestion}</button>
              ))}
            </div>
          </section>
        ) : (
        ...
        <div className="qwen-composer-wrap">
          ...
          <form className="qwen-composer" onSubmit={(event) => void send(event)}>
            <textarea ref={textarea} value={draft} onChange={(event) => setDraft(event.target.value)}
              ...
              disabled={pending || !ready} aria-label="给 Qwen 发送消息" />
            <button type="submit" disabled={!draft.trim() || pending || !ready} aria-label="发送">
              <ArrowUp size={20} />
            </button>
          </form>
