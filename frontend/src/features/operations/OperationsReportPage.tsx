  ...
  return <section aria-labelledby="operations-title" className="page-card operations-page">
    ...
    {selected && <div className="operations-workspace">
      ...
      <div className="operations-detail">
        ...
        {selected.response_plan && <section className="operations-plan" aria-labelledby="operations-plan-title"><header><div><p className="eyebrow">严格结构化建议</p><h3 id="operations-plan-title">响应计划</h3></div><span>{selected.response_plan.execution_status === 'verified_completed' ? '零人工闭环完成' : selected.response_plan.generation_status === 'deterministic_fallback' ? '安全降级' : '编译通过'}</span></header><dl><dt>计划 ID</dt><dd><code>{selected.response_plan.plan_id}</code></dd><dt>计划版本</dt><dd>第 {selected.response_plan.revision} 版</dd><dt>当前状态</dt><dd>{selected.response_plan.status}</dd><dt>动作数</dt><dd>{selected.response_plan.action_count}</dd><dt>执行验证</dt><dd>{selected.response_plan.execution_status === 'verified_completed' ? '模拟动作已执行，执行后状态验证通过' : '请进入处置中心核验'}</dd></dl><p>{selected.response_plan.execution_status === 'verified_completed' ? `隔离回放响应计划已完成零人工闭环；${selected.response_plan.action_count} 项白名单模拟动作均执行成功并通过状态验证。` : selected.response_plan.public_summary}</p>{selected.response_plan.fallback_reason_code && <code>{selected.response_plan.fallback_reason_code}</code>}<small>{selected.response_plan.execution_status === 'verified_completed' ? '该计划由隔离回放零人工策略自动接受、执行并验证，全程保留审计记录。' : '计划生成不代表接受、审批、执行或验证成功；报告保存的是生成时快照。'}</small>{selected.run_id && <div className="operations-plan__links"><Link to={`/response?run_id=${encodeURIComponent(selected.run_id)}`}>进入处置中心</Link></div>}</section>}
