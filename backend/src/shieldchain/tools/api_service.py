...
class TrustedToolApiService:
    ...
    def _trace_item(session: Session, row: TrustedToolCallRow) -> ToolTraceItem:
        ...
        approval_finished_at = (
            _utc(approval.decided_at)
            if approval is not None
            else _utc(policy.created_at)
        ...
        approval_duration_ms = (
            max(
                round(
                    (approval_finished_at - call.request.created_at).total_seconds() * 1000
                ),
                0,
            )
            if approval_finished_at is not None
            else None
        )
        execution_duration_ms = (
            sum(
                max(
                    round(
                        (_utc(item.completed_at) - _utc(item.started_at)).total_seconds()
                        * 1000
                    ),
                    0,
                )
        ...
        if verification is not None and attempts:
            last_completed_at = max(_utc(item.completed_at) for item in attempts)
            verification_duration_ms = max(
                round((_utc(verification.verified_at) - last_completed_at).total_seconds() * 1000),
                0,
            )
