...
MODELS = {
    "qwen3": {"id": "qwen3", "name": "Qwen3-30B-A3B-Instruct-2507-FP8", "served_name": "shieldchain-qwen3-30b"},
...
def catalog() -> dict:
    ...
    if not fresh:
        state = {**state, "phase": "unavailable", "ready": False, "message": "模型控制服务暂不可用，请联系管理员"}
    elif request and request.get("id") != state.get("request_id"):
        state = {**state, "phase": "queued", "target_model": request.get("model"), "ready": False, "message": "已排队，准备切换模型"}
...
def select(model: str) -> dict:
    ...
    with lock():
        ...
        if state.get("phase") in {"switching", "loading", "rollback"} or (queued and queued.get("id") != state.get("request_id")):
