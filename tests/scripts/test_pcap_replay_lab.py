...
def test_plan_is_internal_and_never_uses_host_network() -> None:
    with tempfile.TemporaryDirectory() as temp:
        ...
        assert plan.start_sensor.count("DAC_READ_SEARCH") == 1
        ...
        assert "stream.midstream=true" in plan.start_sensor
        assert "stream.async-oneside=true" in plan.start_sensor
